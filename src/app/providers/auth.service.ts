...
export class AuthService {
  ...
  signInWithGoogle() {
    ...
    /* Filtro para permiso solo a un dominio */
    /* provider.addScope('email');
    provider.setCustomParameters({ hd: 'tudominio.com' }); */
    this.afauth.auth
      ...
      .then(data => {
        ...
        if (this.admin !== true) {
          ...
          this.ngZone
            .run(() => this.router.navigate(["/admin/listado-tickets"]))
            .then();
        }
      })
      .catch(error => {
      ...
      for (let i = 0; i < this.email.length; i++) {
        if (this.nums.indexOf(this.email.charAt(i), 0) !== -1) {
          ...
          this.alerta.toastNotification(
            "Acceso denegado",
            "Aclaraciones: tucorreo@dominio.com.mx",
            "red",
            "fas fa-times"
          );
          return 1;
...
*/
  ...
  */
  verificaUsuarioEnDB() {
    this.apiservice.getFilterUser(this.uid).subscribe((response: any) => {
      /*
      ...
      if (
        this.usuarioLocal.photoURL !== this.photoURL ||
        this.usuarioLocal.displayName !== this.displayName
      ) {
        const user = {
          uid: this.uid,
        ...
        this.usuarioLocal.departamento_id === null ||
        this.usuarioLocal.rol_id === null
      ) {
        const user = {
          uid: this.uid,
          departamento_id: 2,
